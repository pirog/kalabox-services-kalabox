    ...
    kbox.install.registerStep(function(step) {
      ...
      step.all = function(state, done) {
        ...
        .then(function() {
          ...
          if (state.status) {
            state.updateCurrentInstall({SERVICE_IMAGES_VERSION: '0.10.3'});
          }
  ...
  */
  ...
  kbox.install.registerStep(function(step) {
    ...
    step.all.linux = function(state, done) {
      ...
      // Try to cleanup old resolvconf rage
      .then(function() {
        return pkg.exists('resolvconf');
      })

      // If we have resolvconf then try to do the cleanup
      .then(function(exists) {
        if (exists) {
