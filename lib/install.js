...
module.exports = function(kbox) {
  ...
  /*
   * Check to see if our network adapter is set correctly on windows
   * and set it if not
   */
  kbox.install.registerStep(function(step) {
    step.name = 'services-kalabox-windows-check';
    step.description  = 'Configuring Windows host-only adapter';
    step.subscribes = ['core-image-build'];
    step.all.win32 = function(state, done) {

      // Check if we need to add a DNS command
      return util.isHostOnlySet()

      // If not set then set
      .then(function(isSet) {
        if (!isSet) {
          return util.setHostOnly();
        }
      })
